...
// fallback shape as the xfpdGM_* ladder above -- a missing value here must never break a download.
const xfpdVersion = (() => {
    try {
        const info =
            (typeof GM_info !== 'undefined' && GM_info) ||
            (typeof window !== 'undefined' && window.GM_info) ||
            (xfpdGMApi && xfpdGMApi.info) ||
            null;
...
// Measured 2026-07-28 across TM/VM x Chrome/Firefox x native/browser download modes.
//
//   Subfolders work if and only if the handler routes GM_download through the browser
//   downloads API (GM_info.downloadMode === 'browser'). In native mode BOTH handlers flatten
//   the path to underscores silently, so we flatten it ourselves instead and keep control of
//   name length and uniqueness.
//
//   Firefox refuses page-origin blob: URLs from downloads.download, and Violentmonkey resolves
//   URLs background-side rather than reading the blob in the page. So on VM+Firefox GM_download
//   cannot save a blob at all -- in either mode. FileSaver runs in page context, which Firefox
//   permits, so blob saves are routed there. FileSaver is a plain anchor download with no say
//   over the save location, hence no subfolders on Firefox. Tampermonkey is unaffected: it
//   resolves blobs itself, which is why the mainline script works there.
window.isVM = /violentmonkey/i.test(
    (typeof GM_info !== 'undefined' && GM_info && GM_info.scriptHandler) || '',
);

// Read per call, never cached: Violentmonkey pushes a live update when the setting or the
// downloads permission changes, without a page reload.
window.canNestPaths = () => !window.isFF
    && ((typeof GM_info !== 'undefined' && GM_info && GM_info.downloadMode) === 'browser');

window.needsFileSaver = () => window.isVM && window.isFF && typeof saveAs === 'function';

let warnedAboutFlatNames = false;
window.warnIfFlatNames = () => {
    if (warnedAboutFlatNames || window.canNestPaths()) return;
    warnedAboutFlatNames = true;
    if (window.isFF) {
        console.warn(
            '[XenForoSCPDL] Firefox saves everything with flattened names (no subfolders). '
            + 'Firefox blocks extension downloads of page-created blobs, so files are saved via '
            + 'FileSaver, which cannot choose a folder. This is a browser limitation, not a setting.',
        );
    } else {
        console.warn(
            '[XenForoSCPDL] Subfolders are unavailable, so names are being flattened. To get '
            + 'folders: Violentmonkey -> Settings -> Advanced -> tick "Use browser download API" '
            + 'and click "Grant permission" (Tampermonkey: set Download Mode to "Browser API").',
        );
    }
};

// Single entry point for saving an in-memory blob, so the FileSaver split lives in one place.
// NOTE: the FileSaver branch is fire-and-forget -- it reports a HAND-OFF, not a confirmed save.
window.saveBlobAs = (blob, name, handlers) => {
    const cb = handlers || {};
    window.warnIfFlatNames();

    if (window.needsFileSaver()) {
        try {
            saveAs(blob, name);
        } catch (e) {
        ...
        if (cb.onload) cb.onload({ handedOff: true });
        return;
    }

    const blobUrl = URL.createObjectURL(blob);
    const release = fn => arg => {
    ...
    xfpdGM_download({
        ...
        onerror: release(cb.onerror),
        ontimeout: release(cb.ontimeout),
    });
};
// -----------------------------------------------------------------------------------------
window.logs = [];
...
// connection and never answer** -- ~78s each against Chrome's socket timeout.
...
// cookies are not ours to send. Violentmonkey on Firefox has exactly the same problem.
//
...
// tabs.
//
// The direct GM_xmlhttpRequest stays the first attempt: it still works where cookies are not
// partitioned and costs a single request. The tab is only opened when that first attempt comes back
// 403, empty, or unparseable, so nothing changes for users who were never affected.
...
//
// Two reasons, both from contributor testing of b08/vm08. First, empirical: on Firefox +
// Tampermonkey a tab opened at `https://goonbox.cr/` never ran the script at all -- no heartbeat
// ever arrived, so every resolve ate the full 20s readiness timeout and then failed. Opening the
// actual content URL works. The most likely mechanism is that the bare origin redirects somewhere
// the @match does not cover (`www.`, most plausibly), so the script is never injected; hence the
// added wildcard @match as well. This build (Violentmonkey) was *not* affected, but it takes the
// same fix to stay in sync and because the marker gate below is worth having regardless.
//
...
// Violentmonkey does not reliably expose the legacy GM_* functions as bare globals -- the same
// reason this build carries the xfpdGM_* ladder for the transport APIs. Storage and tab access get
// the two *sync-safe* rungs, GM_x then window.GM_x.
//
// The third rung the transport ladder uses (GM.x) is deliberately omitted: GM.getValue/GM.setValue
// are promise-based, and the bridge handshake reads storage synchronously inside its poll loops. A
// promise-returning getValue would make gbxBridgeAlive() truthy on every call and break the
// protocol silently rather than loudly, so falling back to it would be worse than not having it.
const xfpdGetValue =
    (typeof GM_getValue === 'function' ? GM_getValue : null) ||
    (typeof window.GM_getValue === 'function' ? window.GM_getValue.bind(window) : null);
const xfpdSetValue =
    (typeof GM_setValue === 'function' ? GM_setValue : null) ||
    (typeof window.GM_setValue === 'function' ? window.GM_setValue.bind(window) : null);
const xfpdOpenInTab =
    (typeof GM_openInTab === 'function' ? GM_openInTab : null) ||
    (typeof window.GM_openInTab === 'function' ? window.GM_openInTab.bind(window) : null);
...
function gbxGet(key, dflt = '') {
    try { return xfpdGetValue ? xfpdGetValue(key, dflt) : dflt; } catch (e) { return dflt; }
}

function gbxSet(key, val) {
    try { if (xfpdSetValue) xfpdSetValue(key, val); } catch (e) {}
}
...
function gbxCloseTab() {
    ...
    // which matters more here than on the Tampermonkey build).
...
// In b08/vm08 it was armed on settle only, so the 5s timer left by request N could fire in the
// middle of request N+1 and close the tab out from under it -- the in-flight request then saw a
// dead bridge and returned null. Back-to-back album pages usually finished inside the 5s window and
// hid it, but any request slower than that (or one that had to wait out a tab reopen) hit it.
...
function gbxOpenTab(pageUrl) {
    gbxSet(GBX_K_READY, 0);
    try {
        if (!xfpdOpenInTab) return null;
        return xfpdOpenInTab(gbxHelperUrl(pageUrl), { active: false, insert: true, setParent: true });
    } catch (e) {
        return null;
    }
}
...
r = await h.promise(resolve => resolve(resolverCB(resource, h.http, passwords, postId, postSettings, progressCB)));
    ...
    const canNestPaths = window.canNestPaths;
    ...
    batches.push(tmp);
        ...
        while (batch.length) {
            ...
            const startDownload = async (resource, pass = 1) => {
                ...
                // single /d/ links worked from vm06 while albums kept retrying fsc1/fsc2/fsc3 and
                ...
                const startDirectDownload = async (metaHint = null) => {
                    ...
                    try {
                        ...
                        const saveAsName = canNestPaths() ? saveAsPath : saveAsFF;
                            ...
                            try {
                                xfpdGM_xmlhttpRequest({
                                    ...
                                    onload: r => {
                                        ...
                                        window.saveBlobAs(blob, saveAsName, {
                                            onload: () => dlOpts.onload(),
                                            onerror: err => dlOpts.onerror(err),
                                            ontimeout: err => dlOpts.ontimeout(err),
                                        });
                                    },
...
// Success path (unchanged)
                        ...
                        const saveAsName = (!canNestPaths() && !zippedForThis) ? saveAsFF : saveAsPath;

                        if (!zippedForThis) {
                            window.saveBlobAs(fileBlob, saveAsName, {
                                onerror: response => {
